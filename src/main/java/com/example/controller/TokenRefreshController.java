    ...
    public Result<?> refreshToken(@RequestHeader("Authorization") String authHeader) {
        try {
            if (authHeader != null && authHeader.startsWith("Bearer ")) {
                ...
                if (newToken != null) {
                    return new Result<>(200,"success", "Bearer " + newToken);
                } else {
                    return new Result<>(400,"error", "无法刷新 Token");
                }
            } else {
                return new Result<>(400,"error", "无效的 Token 格式");
            }
        } catch (Exception e) {
            // 可以根据具体异常类型进行不同处理，这里统一返回通用错误信息
            return new Result<>(500,"error", "刷新 Token 时发生未知错误: " + e.getMessage());
        }
    }
}
