...
public class MailService {
    ...
    private String buildEmailContent(Object code, String time) {
        // 构建邮件内容的逻辑，与之前相同
        return "<h3>\n" +
    ...
    public void sendEmail(String email, String title, String key) {
        ...
        // 格式化时间
        String time = DateTypeUtil.formatDateTime(ZonedDateTime.now());
        try {
            // 创建简化版邮件消息对象
            SerializableMailMessage mailMessage = new SerializableMailMessage();
            mailMessage.setTo(email);           // 单一收件人
            mailMessage.setSubject(title);
            mailMessage.setText(buildEmailContent(code, time));
    ...
    public void receiveEmailMessage(SerializableMailMessage mailMessage){
        try {
            ...
            helper.setTo(mailMessage.getTo());        // 单一收件人
